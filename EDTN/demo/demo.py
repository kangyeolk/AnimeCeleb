...
from func import loop_iterable, set_requires_grad, GrayscaleToRgb, one_vox_deca_pose, convert_deca_to_angles, get_anime_basis, read_deca
import argparse
...
def get_args():
    ...
    ## required
    parser.add('--path_video', type=str, default='./../example/vox_video/example.mp4')
...
def video2img(path):
    if not os.path.exists(f'./results/{now}/img'):
        os.mkdir(f'./results/{now}/img')

    vidcap = cv2.VideoCapture(path)
    success,image = vidcap.read()
    count = 0
    while success:
        cv2.imwrite(f"./results/{now}/img/%06d.jpg" % count, image)     # save frame as JPEG file
        success,image = vidcap.read()
...
def get_images():
    entire_video= []
    path_img = f'./results/{now}/img'
    frames = sorted(os.listdir(str(path_img)))
    num_frames = len(frames)
    frame_idx = np.arange(num_frames)
    video_array_vox = [img_as_float32(io.imread(os.path.join(path_img, frames[idx].replace('.mat', '.png')))) for idx in frame_idx]
...
if __name__ == '__main__':
    ...
    # entire_video.append(np.array(video_array_vox))
    # entire_video = np.concatenate(entire_video, axis=0)
    dmm_videos = get_images()
    testdata_path = f'./results/{now}/img'
    testdata = datasets.TestData(testdata_path, iscrop=True, face_detector='fan', sample_step=10)
    ...
    for idx in tqdm(range(len(dmm_videos))):
        ...
        test_images = testdata[idx]['image'].to(device)[None,...]
        with torch.no_grad():
            codedict = deca.encode(test_images, use_detail=True)
            codedict = dict_tensor2npy(codedict)
        vox_poses = np.concatenate((codedict['shape'], codedict['exp'], codedict['pose']), axis=-1).squeeze()
        vox_driving_exp = torch.from_numpy(vox_poses).unsqueeze(dim=0).to(device)
