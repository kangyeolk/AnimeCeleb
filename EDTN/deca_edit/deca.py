...
class DECA(nn.Module):
    ...
    def encoder(self, images, use_detail=True):
        if use_detail:
            # use_detail is for training detail model, need to set coarse model as eval mode
            with torch.no_grad():
                parameters = self.E_flame(images)
    ...
    def rendering(self, codedict, input_image, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=False,
                render_orig=True, original_image=None, tform=None):
    ...
    def vertex(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=False,
                render_orig=True, original_image=None, tform=None):
        ## decode
        verts, _, _ = self.flame(shape_params=codedict[:,:100], expression_params=codedict[:,100:150], pose_params=codedict[:,150:156])

        return verts
    
    def landmark2d(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=False,
                render_orig=True, original_image=None, tform=None):

        ## decode
        _, landmark2d, _ = self.flame(shape_params=codedict[:,:100], expression_params=codedict[:,100:150], pose_params=codedict[:,150:156])
        # print('1', landmark2d.shape)
        cam = torch.zeros(codedict.size(0),3).to(codedict.device)
        cam[:,0:3] = torch.tensor([10.0, 0.0, 0.0])
        
        landmark2d = util.batch_orth_proj(landmark2d, cam)[:,:,:2]; landmark2d[:,:,1:] = -landmark2d[:,:,1:] #; landmarks2d = landmarks2d*self.image_size/2 + self.image_size/2
        # print(landmark2d.shape)

        return landmark2d
    
    def landmark3d(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=False,
                render_orig=True, original_image=None, tform=None):

        ## decode
        _, _, landmark3d = self.flame(shape_params=codedict[:,:100], expression_params=codedict[:,100:150], pose_params=codedict[:,150:156])
    
        return landmark3d
    
    def renderers(self, codedict):
        
        batch_size = codedict.shape[0]

        verts, _, _ = self.flame(shape_params=codedict[:,:100], expression_params=codedict[:,100:150], pose_params=codedict[:,150:156])
        
        # albedo = torch.zeros([batch_size, 3, self.uv_size, self.uv_size], device=codedict.device) 
    ...
    def viz_rend_land(self, codedict):
        
        batch_size = codedict.shape[0]

        verts, landmark2d, _ = self.flame(shape_params=codedict[:,:100], expression_params=codedict[:,100:150], pose_params=codedict[:,150:156])
        ...
        width = 256
        height = 256
        channels = 3  # 3 channels for RGB color

        # Create a tensor filled with white color
        white_image = torch.ones((channels, height, width), dtype=torch.float32).to(codedict.device).unsqueeze(0)


        # Optionally, you can scale the pixel values to 0-255 range if you want to save it as an image
        white_image *= 255

        landmark_viz = util.tensor_vis_landmarks(white_image, landmark2d)

        vizdict = {
            'shape_images' : shape_images,
            'landmark_viz' : landmark_viz
        }
